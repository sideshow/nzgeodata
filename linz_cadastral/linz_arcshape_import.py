...
Usage: %s arcshape_root year dbname
    e.g. %s data/Statistics\ NZ/Lv1_2009_Digital\ Boundaries/ARCSHAPE/ 09
        """ % (sys.argv[0],sys.argv[0])
        sys.exit(-1)
    root, year, dbname = sys.argv[1:]
    tables = ['%s%s' % (a, year) for a in ['au', 'mb', 'regc', 'ta', 'ua']]       

    for name in tables:
        shapefile = os.path.join(root, name)
        ...
        os.system("""/usr/bin/env psql -d %s -c "ALTER TABLE %s ADD CONSTRAINT enforce_srid_the_geom CHECK (srid(the_geom) = 2193);" """ % (dbname,name,))
    ...
    for index in indexes:
        os.system("""/usr/bin/env psql -d %s -c "%s" """ % (dbname,index,))
